...
describe("installation", () => {
  ...
  it("installe un service local sans secret intégré", async () => {
    ...
    expect(windowsInstaller).toContain("GetFolderPath(\"Startup\")");
    expect(windowsInstaller).toContain("Démarrer PronoteConnect silencieusement");
    expect(windowsInstaller).not.toContain("Register-ScheduledTask");
    ...
    expect(windowsService).toContain("pronoteconnect.log");
    expect(windowsLauncher).toContain("last-install-error.txt");
    ...
    expect(interfacePage).toContain("tunnel-error");
